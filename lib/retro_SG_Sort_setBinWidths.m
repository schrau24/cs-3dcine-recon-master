function [extr2, nPhases] = retro_SG_Sort(labels,sg,nPhases)
...
% we want equally spaced resp phases:
sgRange = max(sg) - min(sg);
respBins = max(sg):-sgRange/nPhases:min(sg);
...
for ms = 1:nPhases
    if ms < nPhases
        temp_ind = find(sg <= respBins(ms) & sg > respBins(ms+1));
    else
        temp_ind = find(sg <= respBins(ms));
    end
    extr2(temp_ind) = ms-1;
    limits(1,ms) = respBins(ms);
    limits(2,ms) = respBins(ms+1);
end
