    ...
    if isfield(RP,'resp_RetroRespiratoryBinning') && strcmp(RP.resp_RetroRespiratoryBinning, 'Yes')   
        ...
        if isfield(RP,'resp_RetroRespiratoryBinningMethod') && strcmp(RP.resp_RetroRespiratoryBinningMethod, 'SG')
            logger.note(sprintf('Performing self-gating on centre k-space line'));  
                [RespSignal, sg_signal_final] = respiratory_motion(mrecon, 1);
                [extr2, nPhases] = retro_SG_RegistrationSort(mrecon.Parameter.Labels, sg_signal_final, RP.resp_Phases);
        end

        if isfield(RP,'resp_RetroRespiratoryBinningMethod') && strcmp(RP.resp_RetroRespiratoryBinningMethod, 'Belt')
                logger.note(sprintf('Using PEAR belt signal for resp. binning'));
                pear_signal = readPEARSignal(mrecon, RP);
                [extr2, nPhases] = retro_PEAR_RegistrationSort(mrecon.Parameter.Labels, ...
                    pear_signal, RP.resp_Phases);
        end
        ...
        if isfield(RP,'resp_RetroRespiratoryBinningMethod') && strcmp(RP.resp_RetroRespiratoryBinningMethod, 'VitalEye')
            ...
            vEye = vEye/max(vEye(:));
            % sort into nPhases
            [extr2, nPhases] = retro_vEye_Sort(mrecon.Parameter.Labels,vEye,RP.resp_Phases);
        end
...
%% start recon
% check for Serial Profile change == ON, if not, set cross lines to JUNK
% labelling
tmp = mrecon.Parameter.GetParameter('MPF_PROTO_proud_card_enable');
isSerialON = tmp.values;
if ~isSerialON
    logger.note(sprintf('Finding orthogonal cross lines'));
    [mrecon] = removeCrossK0Lines(mrecon);
end
...
if checkRPflag(RP,'viewSharing')
    share = 1;
    ksp = squeeze(mrecon.Data);
    mrecon.Data = [];
    % done for each resp bin separately
    for ii = 1:nPhases
        
        fprintf('\nView sharing for resp bin %i\n',ii);
        ksp(:,:,:,:,:,ii) = fill_kspace3D(ksp(:,:,:,:,:,ii),share);
    end
    mrecon.Data = permute(ksp,[1 2 3 4 7 5 8 9 10 11 6]);
    clear ksp;
end
...
% option to do coil reduction with ROVir (before zero-fill)
% requires a predefined mask over the whole heart
if (checkRPflag(RP, 'ROVir_coils'))
    logger.note(sprintf('Reducing data using ROVir to %i coils',RP.nROVirCoils));
    mrecon.Data = ROVir(mrecon.Data,RP);
    imwrite(frame2im(getframe(figure(22))),[dir_out(RP) '/ROVir_time-averaged.png']);
end
...
if checkRPflag(RP, 'exp_meanResp')
    %%currently losing phase information: could store angle info and add
    %%later?
    ...
    %expirationphase should always be one by check in respiratory_motion.m
    expiration = 1;
...
if checkRPflag(RP,'exp_nii')
	...
	% re-orient for loading outside Matlab
        recon  = flip(rot90(recon,1),1);
        recon2 = make_nii(recon, mrecon.Parameter.Scan.RecVoxelSize);
        save_nii(recon2,[savename '_mag.nii']);
    catch ME
        logger.note(sprintf('WARNING: Error occured in recon.m -- save .nii -- data could not be saved!\n%s',ME.message'));
    end
    if checkRPflag(RP, 'exp_meanResp')
        
        reconmeanresp = abs(meanRESP.Data); reconmeanresp = reconmeanresp/max(reconmeanresp(:));
        reconmeanresp = flip(rot90(reconmeanresp,1),1);
        try
            recon3 = make_nii(reconmeanresp, mrecon.Parameter.Scan.RecVoxelSize);
            save_nii(recon3,[savename '_mag-meanRESP.nii']);
        catch ME
            logger.note(sprintf('WARNING: Error occured in recon.m -- save meanRESP.nii -- data could not be saved!\n%s',ME.message'));
         end
    end
end
