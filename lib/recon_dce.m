...
if checkRPflag(RP,'respSG')
    logger.note(sprintf('Performing self-gating on centre k-space line'));
    [RespSignal, sg_signal_final] = respiratory_motion(RESP);
    [respPhases, nPhases] = retro_SG_Sort(RESP.Parameter.Labels,sg_signal_final,RP.respPhases);
    % save into MRecon object
    RESP.Parameter.Labels.Index.respPhase = respPhases;
...
% do respiratory soft gating after corrections and before sorting
if checkRPflag(RP,'respSG')
    tmpRespPhases = respPhases(mrecon.Parameter.Labels.Index.typ == 1);
    % create soft weights
    for jj = 1:nPhases
        Weight(jj) = (1/4)^(jj-1);
    end
    % loop over phases and apply weights to k-space
    for phase = 0:nPhases-1
        ind = find(tmpRespPhases == phase);
        mrecon.Data(:,ind) = mrecon.Data(:,ind)*Weight(phase+1);
    end
end
...
% temp overwrite
% mrecon.Parameter.Labels.Index.extr2 = zeros(size(extr2));
% mrecon.Parameter.Parameter2Read.extr2 = 0;
mrecon.Parameter.Parameter2Read.Update;
...
try
    if ( ~checkRPflag(RP,'bart_skip_fmac_SENSEUnfold') || ~checkRPflag(RP,'bart_sensemap') )
        ...
        S = MRsense(fullfile(RP.data_dir, RP.data_senseref), mrecon);
