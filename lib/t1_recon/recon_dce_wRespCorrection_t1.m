function recon_dce_wRespCorrection_t1( RP )
...
% for vfa25, have to manually crop the data to 40 seconds
if contains(RP.data_target,'vfa25')
    nchan = numel(RESP.Parameter.Parameter2Read.chan);
    
    % the true TR is the tfe_shot_dur/tfe_factor (includes SPAIR pulses)
    tfe_shot_dur = RESP.Parameter.GetValue('IF_tfe_shot_dur')/1000;     % in seconds
    tfe_factor = RESP.Parameter.GetValue('IF_tfe_factor');              % number of readouts per TFE shot
    trueTR = tfe_shot_dur/tfe_factor;                                   % in seconds
    
    % calculate the readouts for the first 40 seconds, set all others to
    % typ == 10, change label length accordingly
    RESP.Parameter.Labels.Index.typ(round(40/trueTR * nchan)+1 : end) = 10;
    RESP.Parameter.Labels.OriginalLabelLength = round(40/trueTR * nchan);
end
...
load([savename '/respRecon.mat'],'magnitude','magnitude2','registered','extr2');
...
for phase = 1:size(transforms,2)
    ...
    KSP(:,idx) = KSP(:,idx) .* exp(2*pi*1i*(...
        RL_offsets(phase) * squeeze(k(:,idx,1)) + ...
        AP_offsets(phase) * squeeze(k(:,idx,2)) + ...
        FH_offsets(phase) * squeeze(k(:,idx,3)) ...
        ));
end

mrecon.Data = KSP; clear KSP;
%% Perform sorting for DCE recon
extr2  = zeros(size(mrecon.Parameter.Labels.Index.extr2));
nFrames = 1;
disp([ num2str(nFrames) ' frame to reconstruct'])
...
% put DCE frames into 5th dimension, dynamics 
mrecon.Data = permute(mrecon.Data, [1 2 3 4 11 5 6 7 8 9 10]);
mrecon.WritePar([savename, '.par']);
mrecon.WriteRec([savename, '.rec']);

% export dicoms over dynamics dimension
data_storage = mrecon.Data;
for i_dyn = 1:size(mrecon.Data,11)
    mrecon.Data = data_storage(:,:,:,:,i_dyn);
...
if checkRPflag(RP,'exp_nii')
    ...
    try
        addpath(genpath(sprintf('/home/%s/lood_storage/divi/Pub/Software/matlab/toolbox_common/nifti/20140122/',getenv('USER'))))
